...
from dmq.abc.broker import QBrokerProtocol
from dmq.types import BasePeriodicSchedule

from ..types import Schedule, TaskMessage


class InMemoryBroker(QBrokerProtocol):
	def __init__(self, max_concurrent_per_consumer: int = 10) -> None:
		self._task_queue: asyncio.Queue[TaskMessage] = asyncio.Queue()
		self._scheduled_queue: asyncio.PriorityQueue[tuple[float, float]] = asyncio.PriorityQueue()
		self._msgmap: dict[tuple[float, float], TaskMessage] = {}
	...
	async def send_task(
		...
		message = TaskMessage(
			task_id=task_id if task_id else str(ulid()),
			task_name=task_name,
			args=args,
			kwargs=kwargs,
			options=options,
	...
	async def send_scheduled_task(
		...
		key = (execute_at, datetime.now().timestamp())
		self._msgmap[key] = message
		await self._scheduled_queue.put(key)
	...
	async def _process_scheduled_tasks(self) -> None:
		while self._running:
			...
			execute_at, _hash = await self._scheduled_queue.get()
			now = datetime.now(UTC).timestamp()

			if now >= execute_at:
				message = self._msgmap.pop((execute_at, _hash))
				await self._task_queue.put(message)
				logger.debug(
					"scheduled task (execute_at={}, queued_at={}) {} ready for execution",
					execute_at,
					_hash,
					message.task_id,
				)
			else:
				await self._scheduled_queue.put((execute_at, _hash))
	...
	async def consume(self) -> AsyncIterator[TaskMessage]:
		...
		:rtype: AsyncIterator<TaskMessage>
		"""
