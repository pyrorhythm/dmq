...
from ..utils import get_type_fqn, get_type_from_fqn
...
class RedisResultBackend:
	...
	async def get_result(self, task_id: str, timeout: float | None = None) -> object:
		key = f"dmq:result:{task_id}"
	...
	async def _fetch_result(self, key: str) -> object:
		_deser = None
		...
		if data is not None:
			...
			_deser = self.serializer.deserialize(data, into=get_type_from_fqn(_result))
