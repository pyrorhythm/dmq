...
class QWorkerPool:
	def __init__(
		...
		self.workers: Sequence[QAsyncWorker | QThreadedWorker] = []
		self._worker_tasks: Sequence[asyncio.Task] = []
	...
	def _create_workers(self) -> Sequence[_Worker]:
	...
	async def start(self) -> None:
		...
		if self.execution_mode == ExecutionMode.THREADED:
			threaded_workers = cast(Sequence[QThreadedWorker], self.workers)
			...
			async_workers = cast(Sequence[QAsyncWorker], self.workers)
			self._worker_tasks = [
	...
	async def shutdown(self, timeout: float = 30.0) -> None:
		...
		if self.execution_mode == ExecutionMode.THREADED:
			with ThreadPoolExecutor(max_workers=len(self.workers)) as executor:
				futures = [
					executor.submit(partial(worker.stop, timeout=timeout))
					for worker in cast(Sequence[QThreadedWorker], self.workers)
				]
				for future in futures:
					try:
						future.result(timeout=timeout)  # pyrefly:ignore[unused-coroutine]
					except Exception as e:
						logger.warning("error stopping worker: {}", e)
		else:
			try:
				await asyncio.wait_for(
					asyncio.gather(*tuple(worker.stop() for worker in cast(Sequence[QAsyncWorker], self.workers))),  # type:ignore
					timeout=timeout,
				)
			except TimeoutError:
