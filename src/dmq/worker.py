...
class QWorker:
    ...
    async def _handle_task(self, message: TaskMessage) -> None:
        ...
        if self.delivery_config.should_check_idempotency():
            if await self.idempotency_store.is_processed(message.task_id):  # ty:ignore[possibly-missing-attribute]
                await self.manager.broker.ack_task(message.task_id)
                return
        ...
        try:
            ...
            if self.delivery_config.should_check_idempotency():
                await self.idempotency_store.mark_processed(message.task_id, time.time())  # ty:ignore[possibly-missing-attribute]
